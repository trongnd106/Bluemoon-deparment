...
export default function Cards() {
  ...
  useEffect(() => {
    ...
    const totalResidents = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/v1/residents?size=999");
    ...
    const totalVehicles = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/v1/vehicles?size=999");
