...
const StatisticsForm = ({ statistic }: StatisticsFormProps) => {
  ...
  const apiInvoice = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/v1/invoiceapartment/${addressNumber}`);
  ...
  const apiPayInvoice = async (invoiceId: string, feeList: any[]) => {
    try {
      ...
      // Gửi request API
      await axios.put(`http://localhost:8080/api/v1/invoiceapartment/update/${addressNumber}/${invoiceId}`, payload, {
  ...
  const apiUtility = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/v1/utilitybills/${addressNumber}`);
  ...
  const apiPayUtility = async (utilityId: string) => {
    try {
      ...
      // Đoạn này
      await axios.post(`http://localhost:8080/api/v1/utilitybills/update/${utilityId}`);
        ...
        {dataInvoice.map((invoice, index) => (
          <div key={index}>
            ...
            >
              ...
              <span className="status">{invoice.paymentStatus === "Unpaid" ? "Unpaid" : "Paid"}</span>
              <span className={`arrow ${openDropdowns[invoice.id] ? "open" : ""}`}>&#9662;</span>
            </div>
                  ...
                  <tbody>
                    {invoice.feeList.map((fee: any, feeIndex: number) => (
                      <tr key={feeIndex}>
                        ...
                        <td>
                          {fee.name === "Fund 2"
                            ? (fee.amount + (voluntaryFund[invoice.id] || 0)).toLocaleString()
                    ...
                    (fee: any) =>
                      fee.feeType === "ContributionFund" && (
                        <div className="voluntary-fund" key={fee.name}>
                          <label>
                            Enter voluntary contribution for <strong>{fee.name}</strong>:{" "}
                            <input
                              className="inputFund"
                              type="text"
                  ...
                  {invoice.feeList
                    .reduce((sum: number, fee: any) => {
                      if (fee.feeType === "DepartmentFee") {
                        return sum + fee.amount + (voluntaryFund[invoice.id] || 0);
                      }
        ...
        {dataUtility.map((utility, index) => (
          <div key={index}>
            ...
            >
              ...
              <span className="status">{utility.paymentStatus === "Unpaid" ? "Unpaid" : "Paid"}</span>
              <span className={`arrow ${openDropdowns[utility.id] ? "open" : ""}`}>&#9662;</span>
            </div>
                ...
                <table>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>{(utility.electricity + utility.water + utility.internet).toLocaleString()} VND</td>
                    </tr>
                  </tbody>
                </table>
                <div className="divPay">
                  <button onClick={() => apiPayUtility(utility.id)} className="payButton" type="submit">
                    Pay <i className="bx bxs-credit-card-alt"></i>
                  </button>
                </div>
              </div>
