...
const AccoutForm = () => {
  ...
  const handleUpdate = async (e: any) => {
    ...
    try {
      const response = await axios.put("http://localhost:8080/api/v1/users", data);
    ...
    <Form width="400px">
      <Form.Fields>
        <FormField>
          ...
          <FormField.Input id="email" type="text" value={data.email} readOnly onChange={() => {}} />
        </FormField>
        <FormField>
          <FormField.Label label="Name:" />
          <FormField.Input id="name" type="text" onChange={handleChange} value={data.name} />
        </FormField>
        <FormField>
