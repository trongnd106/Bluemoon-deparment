...
export default function ApartmentForm({ apartment, fetchApartments }: ApartmentFormProps) {
  ...
  const [selectedResidents, setSelectedResidents] = useState<Resident[]>(apartment?.residentList || []);
  ...
  const handleResidentsSelect = (newResidents: Resident[]) => {
    const updatedResidents = [
      ...selectedResidents,
      ...newResidents.filter(
        (newResident) => !selectedResidents.some((existingResident) => existingResident.id === newResident.id)
      ),
    ];
  ...
  const handleUpdate = async (e: any) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8080/api/v1/apartments/${formValues.addressNumber}`, data);
  ...
  return (
    <Form width="800px">
      ...
      <Selector id="status" value={formValues.status} onChange={handleChange} options={statusOptions} label="Status" />
