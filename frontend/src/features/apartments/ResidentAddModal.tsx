...
export default function ResidentAddModal({ onResidentsSelect }: ResidentAddModalProps) {
  ...
  const apiAllResidents = async () => {
    try {
      ...
      setSuggestions(residentsList.map((resident: any) => resident.name)); // Chỉ lưu danh sách tên cho gợi ý
    } catch (err) {
    ...
    <div className="wrp">
      <div className="searchDiv">
        ...
        <div className="search-ctn">
          ...
          />
          <ul id="suggestions" className={`suggestions ${suggestions.length === 0 ? "hidden" : ""}`}>
            {suggestions.map((name, index) => (
              <li key={index} onClick={() => handleSelect(name)} className="suggestion-item">
                {name}
              </li>
            ))}
          </ul>
          ...
          {selectedResidents.length > 0 ? (
            selectedResidents.map((resident) => (
              <div key={resident.id} className="selectedResidentItem">
                ...
                <span className="remove-icon" onClick={() => handleRemove(resident.id)}>
                  &times;
                </span>
              </div>
            ))
