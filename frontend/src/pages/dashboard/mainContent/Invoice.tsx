...
function InvoiceTDN() {
  ...
  const addFee = () => {
    const selectedFee = feeOptions.find((fee) => fee.name === formValues.feeType);
  ...
  const addFund = () => {
    const selectedFund = fundOptions.find((fund) => fund.name === formValues.fundType);
  ...
  const saveForm = async (event: React.FormEvent) => {
    ...
    try {
      // Gửi dữ liệu tới API
      await axios.post("http://localhost:8080/api/v1/invoices", payload);
        ...
        <div style={invoiceStyles.leftColumn}>
          <Form.Fields>
            <FormField>
              ...
              <FormField.Input id="id" type="text" value={formValues.id} onChange={handleChange} />
            </FormField>

            <FormField>
              <FormField.Label label={"Name"} />
              <FormField.Input id="name" type="text" value={formValues.name} onChange={handleChange} />
            </FormField>
          </Form.Fields>

          <div style={invoiceStyles.row}>
            <label className="font-bold">Fee: </label>
            <select style={invoiceStyles.input} id="feeType" value={formValues.feeType} onChange={handleChange}>
            ...
            <button type="button" style={invoiceStyles.addButton} onClick={addFee}>
              Add Fee
            </button>
          </div>
          ...
          <div style={invoiceStyles.row}>
            ...
            <select style={invoiceStyles.input} id="fundType" value={formValues.fundType} onChange={handleChange}>
              <option value="">Select</option>
            ...
            <button type="button" style={invoiceStyles.addButton} onClick={addFund}>
              Add Fund
            </button>
          </div>

          <label>Description: </label>
          <Form.TextArea id="description" value={formValues.description} onChange={handleChange} />
        </div>
...
const invoiceStyles: { [key: string]: CSSProperties } = {
  container: {
    position: "relative",
    top: "8px",
    padding: "10px",
